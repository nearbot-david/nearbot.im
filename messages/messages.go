...
func Withdraw(balance int) string {
	return "<b>Withdraw NEAR</b>\n\n" +
		fmt.Sprintf("How much do you want to withdraw? Please note that the fee will be %.1f%% of the amount.\n\n", config.Fee*100) +
...
func WithdrawLowBalance(balance int) string {
	return "<b>Not enough NEAR</b>\n\n" +
		"You don't have enough NEAR to withdraw.\n" +
		fmt.Sprintf("<b>Min amount:</b> %.5f NEAR\n\n", utils.GetMinWithdrawAmount()) +
		fmt.Sprintf("Current balance: <b>%s NEAR</b>\n\n", utils.DisplayAmount(balance))
}
...
func WithdrawIncorrectAmount(balance int) string {
	return "<b>Withdraw NEAR</b>\n\n" +
		"How much do you want to withdraw? For example 0.2\nIf you don't want to withdraw, just send \"cancel\" in response.\n\n" +
		fmt.Sprintf("<b>Min amount:</b> %.5f NEAR\n", utils.GetMinWithdrawAmount()) +
		fmt.Sprintf("<b>Max amount:</b> %.5f NEAR\n\n", calculateMaxWithdrawAmount(balance)) +
		fmt.Sprintf("Current balance: <b>%s NEAR</b>\n\n", utils.DisplayAmount(balance))
}
...
func WithdrawConfirmAmount(amount int, balance int) string {
	return "<b>Withdraw NEAR</b>\n\n" +
		fmt.Sprintf("You are going to withdraw <b>%s NEAR</b>. Please send us the address of the wallet to which you want to receive the NEAR.\n", utils.DisplayAmount(amount)) +
...
func WithdrawConfirmFinal(amount int, wallet string, balance int) string {
	return "<b>Withdraw NEAR</b>\n\n" +
		fmt.Sprintf("You are going to withdraw <b>%s NEAR</b>.\n", utils.DisplayAmount(amount)) +
		fmt.Sprintf("Wallet: <b>%s</b>.\n\n", wallet) +
		fmt.Sprintf("<b>Fee:</b> %s NEAR\n", utils.DisplayAmount(int(float64(amount)*config.Fee))) +
		fmt.Sprintf("Continue? <b>%s NEAR</b>  will be deducted from your balance (fee inc.)\n\n", utils.DisplayAmount(int(float64(amount)*(1+config.Fee)))) +
		fmt.Sprintf("Current balance: <b>%s NEAR</b>\n\n", utils.DisplayAmount(balance))
}

func WithdrawHasProcessing(amount int, wallet string, withdrawalID string) string {
	return "<b>You have active withdrawal</b>\n\n" +
		fmt.Sprintf("At the moment you already have an active withdrawal request.\n\n") +
		fmt.Sprintf("Status: <b>processing</b>\n") +
		fmt.Sprintf("Amount: <b>%s NEAR</b>.\n", utils.DisplayAmount(amount)) +
		fmt.Sprintf("Wallet: <b>%s</b>.\n\n", wallet) +
		fmt.Sprintf("Fee: <b>%s NEAR</b>\n\n", utils.DisplayAmount(int(float64(amount)*config.Fee))) +
		fmt.Sprintf("<i>Withdrawal ID: %s</i>", withdrawalID)
}

func WithdrawCreated(amount int, wallet string, withdrawalID string) string {
	return "<b>Withdrawal create</b>\n\n" +
		fmt.Sprintf("Note that you can only create one withdrawal at a time. To create a new one, wait until processing is complete. This usually takes (1-2 minutes).\n\n") +
		fmt.Sprintf("Amount: <b>%s NEAR</b>.\n", utils.DisplayAmount(amount)) +
		fmt.Sprintf("Wallet: <b>%s</b>.\n\n", wallet) +
		fmt.Sprintf("Fee: <b>%s NEAR</b>\n\n", utils.DisplayAmount(int(float64(amount)*config.Fee))) +
		fmt.Sprintf("<i>Withdrawal ID: %s</i>", withdrawalID)
...
func WithdrawUnexpectedError() string {
	return "<b>Unexpected error</b>\n\n" +
		...
		"Try to withdraw again or contact support if this is not the first time you have seen this message."
}
